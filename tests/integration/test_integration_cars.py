...
def test_get():
    request = requests.get(API_URL + "/cars-api/")
...
def test_get_detail():
    request = requests.get(API_URL + "/cars-api/11/")
...
def test_user_has_no_permission():
    request = requests.post(
        API_URL + "/cars-api/1/",
        json={"car_type": 1, "color": "Color", "year": 2024, "image": ""},
    )
...
def test_update(api_client):
    request = api_client.put(API_URL + "/cars-api/11/", headers=headers)
...
def test_delete(api_client):
    request = api_client.delete(API_URL + "/cars-api/11/", headers=headers)
