...
def test_get():
    request = requests.get(API_URL + "/dealers-api/")
...
def test_get_detail():
    request = requests.get(API_URL + "/dealers-api/1/")
...
def test_user_has_no_permission():
    request = requests.post(
        API_URL + "/dealers-api/1/",
        json={"name": "Name", "available_car_types": [1]},
    )
...
def test_bad_request(api_client):
    request = api_client.post(API_URL + "/dealers-api/", json={}, headers=headers)
...
def test_delete(api_client):
    request = api_client.put(API_URL + "/dealers-api/1/", headers=headers)
